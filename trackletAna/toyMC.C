...
void toyMC(char *inf, int run,int nevt);

void toyMCs(char *inf, int start,int end)
{
...
//  trackletTree->Branch("phi",tdata.phi,"phi[npart]/F");
    ...
    for (int i=0;i<nevt;i++) {
       ...
       hits1.clear();
       hits2.clear();
       if (i% 1000 == 0 ) cout <<"Run: "<<run<<" Event "<<i<<" "<<(int)mult<<" "<<(int)nhit1<<" "<<hits1.size()<<" "<<(int)nhit2<<" "<<hits2.size()<<endl;
