...
//  char *varM =  Form("(Sum$(abs(eta1)<%f&&abs(dR)<0.1))-(Sum$(abs(eta1)<%f&&abs(dR)>0.1&&abs(dR)<0.2))",_etaRange,_etaRange);
  ...
  // Histograms
  TH1D *hMMC = new TH1D("hMMC","",nBin,nTrackletMin,nTrackletMax);
...
//  if (useCorrectionFile=="") {  
     ...
     t->Draw(Form("%s>>hT",varT),nonZeroCut);
  ...
  bayesianUnfold myUnfolding3(hR,hM,0);
  myUnfolding3.unfold(hM,4);
  bayesianUnfold myUnfolding2(hR,myUnfolding3.hUnfolded,0);
  myUnfolding2.unfold(hM,4);

//  bayesianUnfold myUnfolding(hR,myUnfolding2.hUnfolded,0);
