...
TH1D* plotSubtracted(char* filename,char *toyMC,char *mycut,bool useMC)
{
   ...
   TH2F *h1 = new TH2F("h1","",12,-3,3,50,0,1);
   TH2F *h2 = new TH2F("h2","",12,-3,3,50,0,1);
   TH2F *h2MC = new TH2F("h2MC","",12,-3,3,50,0,1);
   TH2F *h3 = new TH2F("h3","",12,-3,3,50,0,1);
 
   TCut cut = mycut;
   TCanvas *c = new TCanvas ("c","",400,400);
   ntmatched->Draw("abs(dphi):eta1>>h1","abs(dphi)<1"&&cut);

   h3->SetXTitle("#eta");
   h3->SetYTitle("|#Delta#phi|");
   if (useMC) {
     cout <<"Use ToyM"<<endl;
     ntmatchedMC->Draw("abs(phi):eta1>>h2","abs(dphi)>1&&abs(dphi)<2"&&cut);
