...
// Main Routine
//===========================================================================
int plotBetaFromTreeVzBin(char* filename,char *toyMC="pseudoExp.root",int useCorrectionFile = 0,char *mycut="",bool useMC = false)
{
   ...
   TrackletTree->Project("hEverything","vz[1]:mult:eta1",signalRegion);
   ...
   if (useMC) {
     ...
     int n = TrackletTree->GetEntries(sideBandRegionEta&&cut);
     int n2 = ntmatchedMC->GetEntries(sideBandRegionEta&&cut);
     h2->Scale(1./n2*n); 
   } else {
   TrackletTree->Project("h2","vz[1]:mult:eta1",sideBandRegionEtaSignalRegion&&cut);   
   }
...
//   h3->Draw("col");
 ...
 //  return 0;
   ...
   TrackletTree->Project("hHadron","vz[1]:mult:eta","abs(eta)<2.5");
...
//   formatHist(hTruth,1,nevent/nEtaBin*6);
   ...
   TCanvas *cRatioEtaNhit = new TCanvas("cRatioEtaNHit","",400,400);
   hEtaHitRatio->Draw("colz");
   TCanvas *cRatioVzNhit = new TCanvas("cRatioVzNHit","",400,400);
