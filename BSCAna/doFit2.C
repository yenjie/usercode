...
double* doFitDeltaT(char *infname = "test.root",int ch1=0,int ch2=1)
{
   ...
   char *cut = Form("min[%d]<-0.1*256*0.86&&min[%d]<-0.1*256*0.86",ch1,ch2);
//   char *cut = "min[0]<-10&&min[1]<-10&&min[2]<-10&&min[3]<-10&&min[4]<-10&&min[5]<-10&&min[6]<-10&&min[7]<-10";

   data->Draw(Form("mintime0802[%d]-mintime0802[%d]>>h",ch1,ch2),Form("mintime0802[%d]>0&&mintime0802[%d]>0&&%s",ch1,ch2,cut));
   cout <<Form("min[%d]<-100&&min[%d]<-100",ch1,ch2);
   
   TF1 *fun = new TF1("fun",myfun,-100,100,4);
   fun->SetParameters(h->GetMaximum(),h->GetMean(),2,0);
...
void doFit2(char *infname = "test.root")
{
   ofstream of;
