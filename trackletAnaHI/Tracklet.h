...
{
   public:
      Tracklet(float eta1,float eta2,float phi1,float phi2,float r1,float
      r2, float cs1, float cs2);
      ~Tracklet(){};

      float eta1() { return eta1_; }
      float eta2() { return eta2_; }
      float phi1() { return phi1_; }
      float phi2() { return phi2_; }
      float r1()   { return r1_; }
      float r2()   { return r2_; }
      float cs1()   { return cs1_; }
      float cs2()   { return cs2_; }
      

      float deta() { return eta1_-eta2_; }
      float dphi();
      float dR();
      float dR2();
...
Tracklet::Tracklet(float eta1, float eta2, float phi1, float phi2, float r1, float r2, float cs1, float cs2) {
...
vector<Tracklet> recoProtoTracklets(vector<RecoHit> hits1, vector<RecoHit> hits2, bool reproduceBck, TH3I *h, ULong64_t &nTracklet, ULong64_t &mult)
{
  ...
  sort (hits2.begin(),hits2.end(),compareEta);
...
void setTrackletTreeBranch(TTree* trackletTree,TrackletData &tdata)
{
  ...
  trackletTree->Branch("mult",&tdata.mult,"mult/l");
  trackletTree->Branch("nProtoTracklet",&tdata.nProtoTracklet,"nProtoTracklet/l");
  ...
  trackletTree->Branch("weight",tdata.weight,"weight[nTracklets]/l");
  
  trackletTree->Branch("npart",&tdata.npart,"npart/I");
  trackletTree->Branch("eta",tdata.eta,"eta[npart]/F");
  trackletTree->Branch("weight2",tdata.weight2,"weight2[npart]/l");
...
//  trackletTree->Branch("nhad",tdata.nhad,"nhad[12]/F");
  ...
  trackletTree->Branch("eb",&tdata.eb,"eb/F");
  trackletTree->Branch("eep",&tdata.eep,"eep/F");
  trackletTree->Branch("eem",&tdata.eem,"eem/F");
  trackletTree->Branch("cBin",&tdata.cBin,"cBin/I");
  trackletTree->Branch("nbins",&tdata.nbins,"nbins/I"); 
  trackletTree->Branch("binsize",&tdata.binsize,"binsize/I");
  trackletTree->Branch("nparti",&tdata.nparti,"nparti/F");
  trackletTree->Branch("npartiSigma",&tdata.npartiSigma,"npartiSigma/F");
  trackletTree->Branch("ncoll",&tdata.ncoll,"ncoll/F");
  trackletTree->Branch("ncollSigma",&tdata.ncollSigma,"ncollSigma/F");
  trackletTree->Branch("nhard",&tdata.nhard,"nhard/F");
  trackletTree->Branch("nhardSigma",&tdata.nhardSigma,"nhardSigma/F");
  trackletTree->Branch("b",&tdata.b,"b/F");
  trackletTree->Branch("bSigma",&tdata.bSigma,"bSigma/F");
