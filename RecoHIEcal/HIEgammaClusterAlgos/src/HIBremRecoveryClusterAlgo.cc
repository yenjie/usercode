...
reco::SuperClusterCollection HIBremRecoveryClusterAlgo::makeSuperClusters(reco::BasicClusterRefVector & clustersCollection)
{
  ...
  for (it = clustersCollection.begin(); it != clustersCollection.end(); it++)
    {
      ...
      if (cluster_p->algo() == reco::island) 
      {
         ...
         {
            if (cluster_p->energy()>barrelBasicClusterEnergyThreshold) {
	       islandClustersBarrel_v.push_back(cluster_p);
	    }
         }
	 else
	 {
	    if (cluster_p->energy()>endcapBasicClusterEnergyThreshold) {
...
void HIBremRecoveryClusterAlgo::makeIslandSuperClusters(reco::BasicClusterRefVector &clusters_v, 
...
bool HIBremRecoveryClusterAlgo::match(reco::BasicClusterRef seed_p, 
