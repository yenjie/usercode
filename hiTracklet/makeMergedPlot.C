...
void clearBin0(TH1D* h)
{
   for (int i=2;i<=11;i++)
   {
      ...
      double e1 = h->GetBinContent(i)*6.0/100;
...
void clearBin(TH1D* h)
{
   ...
   {
      if (h->GetBinContent(i)>0) cout <<i<<" "<<h->GetBinError(i)/h->GetBinContent(i)<<endl;
      h->SetBinError(i,h->GetBinContent(i)*6.0/100);
   }   
...
TH1D* makeMergedPlot(string name = "D6T-Official-Reweight",int UA5=0, double
uncert = 6.0,int par=0,string title= "")
{
   TH1D* hAvg2;
   // layer1+2
...
//   if (UA5>=3) hTracklet2360GeVHF1->Draw("p same");   
   ...
   TH1D *hPixelCounting7TeV = pixelCounting7TeV();  
